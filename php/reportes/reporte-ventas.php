<?php
    ...
    class PDF extends FPDF {
        ...
        public function header() {
            ...
            $this->Ln(10);
            $this->Cell(1);
            $this->Cell(260, 10,'REPORTE GENERAl DE VENTAS POR PRODUCTOS', 0, 0, 'C');
    ...
    $SQL = "SELECT ven.id, usu.nom_usuario, pro.nombre, det.cantidad, det.precio, det.total, ven.fecha, det.mayoreo FROM ventas ven, detalle_venta det, usuarios usu, productos pro WHERE pro.id=det.productos_id AND usu.id=ven.usuarios_id AND ven.id=det.ventas_id AND ven.estatus_id=6 AND (fecha >= '".$fechaInicial." 00:00:00' AND fecha <= '".$fechaFinal." 23:59:59')";
    ...
    //for($i=0 ; $i<100 ; $i++) {
        ...
        if ($contador==0){
            ...
            // Imprimimos el Campo
            $pdf->Cell(84,7,'Producto',1,1,'C',t);
            $pdf->SetXY(155, $separacion);
            // Imprimimos el Campo
            $pdf->Cell(34,7,'Fecha Comp.',1,1,'C',t);
            $pdf->SetXY(189, $separacion);
            // Imprimimos el Campo
            $pdf->Cell(25,7,'Cant.',1,1,'C',t);
            $pdf->SetXY(214, $separacion);
            // Imprimimos el Campo
            $pdf->Cell(30,7,'P/Vent.',1,1,'C',t);
        ...
        // Asigna la Posicion de los Campos
        $pdf->SetXY(71, $separacion);
        $pdf->Cell(84,5,utf8_decode($value->nombre),1,1,'L');
        // Asigna la Posicion de los Campos
        $pdf->SetXY(155, $separacion);
        
        $fecha = new DateTime($value->fecha);
        
        $complementoMayoreo = "p/u";
        if($value->cantidad) {
            $complementoMayoreo = "p/m";
        }
        
        // Imprimimos el Campo
        $pdf->Cell(34,5,utf8_decode($fecha->format('d/m/Y H:m:s')),1,1,'C');
        $pdf->SetXY(189, $separacion);
        // Imprimimos el Campo
        $pdf->Cell(25,5,utf8_decode($value->cantidad),1,1,'C');
        $pdf->SetXY(214, $separacion);
        // Imprimimos el Campo
        $pdf->Cell(30,5,utf8_decode("$".$value->precio." ".$complementoMayoreo." "),1,1,'R');
        $pdf->SetXY(244, $separacion);
        // Imprimimos el Campo
